# Guess Who — 界面与状态清单（设计需求）

> **用途**：给 UI/UX 设计走查用。按**用户操作顺序**写：读到哪一步，就知道用户在哪个界面、画面处于什么状态、各区域与按钮如何变化。  
> **玩法规则**见 [`GAME_DESIGN.md`](GAME_DESIGN.md) 第一节「各模式游玩说明」。
...
| 用语 | 含义 |
|------|------|
| **界面** | 用户看到的整屏页面（如「首页」「游戏页」） |
| **界面状态** | 同一界面在不同情况下的整体样子（如「还没进房间 / 已在房间等待」） |
| **区域** | 界面里的固定区块（顶栏、主内容、底栏等） |
| **区域状态** | 某区域随流程变化的样子（如按钮可点 / 置灰） |
| **弹层** | 盖在界面上的浮层（确认框、倒计时、揭晓答案等） |

阅读顺序：**用户操作 → 所在界面 + 界面状态 → 区域结构 → 按钮可点与否 →（如有）弹层**。
...
| 名称 | 什么时候出现 | 底部按钮 |
|------|--------------|----------|
| 解散房间确认 | 房主点「解散房间」 | 「取消」/ 红色「确认解散」→ 所有人回首页 |
| 开始倒计时 | 房主点「开始游戏」 | 无，自动消失 |
...
**所在界面**：多人结算页  
**什么时候来**：正常打完 / 有人退出 / 自己点「退出」

```
多人结算页
├── 标题「本局结算」
├── 一行灰色结束说明              【如「十道题已完成」「某某已退出」】
├── 【异常结束时】正确答案卡片
...
**平台侧（以后对接）**：天梯邀请房创建后，同房间好友在**平台弹窗**里收到邀请，点进来走上面流程。
...
| 现在游戏里 | 平台版建议 |
|------------|------------|
| 首页手填昵称 | 显示平台头像和昵称，不可改 |
| 六位房间号 | 可保留样式，或改成平台房间号样式 |
| 天梯房紫色条 + 再次邀请 | 和平台全局邀请弹窗风格统一 |
| 单人排行榜 | 保留 |
| 多人结算 | 仍只做房间内排名，不做全服榜 |

---

*文档说明：全文按用户操作顺序与界面中文描述编写，便于设计走查；与当前版本一致（五人房间、房主开始、天梯邀请、解散与退出）。*
